...
## v1.4.2 — 2026-06-16 · PWA 支持

- **添加到手机桌面**：新增 `manifest.json` + `sw.js`，用手机浏览器打开网站后点「添加到主屏幕」即可像 App 一样使用
- 全屏模式（`display: standalone`）、主题色（#ff9aa2）、iOS Safari 适配
- Service Worker 离线缓存：静态资源缓存优先，API 永远走网络
- 仪表盘、记忆管理页面同步支持 PWA
