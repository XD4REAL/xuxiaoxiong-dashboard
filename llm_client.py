...
def extract_memories(user_message: str, reply: str) -> list[dict]:
    ...
    prompt = (
        "从对话中提取值得记住的具体信息。只提取以下三类：\n"
        "①许小熊的设定/规则 ②小多和小豆的共同回忆（去哪、做什么、约定）③小豆纠正你的错误。\n\n"
        "规则：\n"
        "- f必须是具体唯一标识（如\"哈尔滨雪翅膀\"\"棋士谐音\"），严禁使用分类标签（如\"共同回忆\"\"小豆纠正错误\"\"设定\"\"喜好\"）\n"
        "- v是具体细节描述（≤25字）\n"
        "- 禁止记录：当前日期、许小熊自身情绪、元信息\n"
        "- 同一话题只提取最重要的一条，不重复已有信息\n"
        "- 无关紧要的闲聊不提取（如\"唱歌可爱\"\"好听\"这类主观评价）\n"
        "- 没有值得记住的就返回空数组\n\n"
        f"用户：{user_message}\n"
        f"许小熊：{reply}\n\n"
        '返回JSON：{"m":[{"f":"具体事实名","v":"具体值"},...]} 无则{"m":[]}。只返回JSON。'
    )
    try:
        resp = _post_with_retry(
            f"{DEEPSEEK_BASE_URL}/chat/completions",
