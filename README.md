...
- **PWA 支持** — 新增 manifest + Service Worker，手机可添加到桌面全屏使用
- **记忆系统重写** — 三层防线：正则兜底 + LLM 提取 + 质量过滤（拒绝分类标签、近重复、f===v）
- **记忆只取用户消息** — AI 回复仅供理解上下文，不从中提取事实，杜绝 AI 瞎编变记忆
- **风格调整** — 短句为主（1-3句）、去掉括号动作描写、保持变化不套路
- **模型升级** — deepseek-chat → deepseek-v4-flash（7月24日前迁移）
- **新增页面** — `/memories` 记忆管理（浏览/删除）
